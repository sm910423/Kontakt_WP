...
class CASE27_Reports {
    ...
    public function report_listing()
    {
        ...
        if ( ! is_user_logged_in() || ! $user_id || ! $listing_id ) {
            echo json_encode( [
                'status' => 'unauthorized',
                'message' => __( 'You need to be logged in to perform this action.', 'my-listing' )
            ] );
            die;
        }
        ...
        if ( $report_exists ) {
            echo json_encode( [
                ...
                'message' => __( 'You\'ve already reported this listing. It is currently being reviewed.', 'my-listing' )
            ] );
            die;
        }
        ...
        if ( $report_id ) {
            echo json_encode( [
                'status' => 'success',
                'message' => __( 'Your report was submitted successfully. It will be reviewed by our team.', 'my-listing' )
            ] );
            die;
        }
        ...
        echo json_encode( [
            'status' => 'invalid_request',
            'message' => __( 'There was an error with processing your request.', 'my-listing' )
        ] );
        die;
    }
}
